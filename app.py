    ...
    try:
        ...
        def pick_col(name: str):
            if name in df.columns:
                return df[name]
            # tolerate variants like 'GLD_High' or 'High'
...
def fetch_ohlc(start: date, end: date, source: str, ticker: str) -> pd.DataFrame:
    """Unified fetch. For ETH-USD honors crypto fallbacks; others use Yahoo."""
    kind = ASSETS.get(ticker, {}).get("kind", "etf")
    if kind == "crypto" and ticker.upper() == "ETH-USD":
        # Use callables that all share the same (start, end) signature
        order = {
            "Yahoo Finance": [
            ...
            "CoinGecko": [
                # fallback path uses Yahoo if needed (kept simple)
                (lambda s, e: fetch_ohlc_yahoo_generic("ETH-USD", s, e)),
            ],
        }.get(source, [
            fetch_ohlc_coinbase, fetch_ohlc_kraken, fetch_ohlc_binance,
            (lambda s, e: fetch_ohlc_yahoo_generic("ETH-USD", s, e))
        ])
        for fn in order:
            df = fn(start, end)
...
summary, details = simulate_strategy(
    ...
    start_value_usd=float(start_value_usd), ref_price_usd=float(ref_price_usd),
...
if ASSETS[ticker]["kind"] == "crypto" and ticker == "ETH-USD":
    ...
    spot_price, spot_src = fetch_spot_yahoo_close(ticker)
a2.metric(f"{ASSETS[ticker]['label'].split('(')[0].strip()} price (now)",
...
st.subheader(f"{ticker} Price with Buys & Sells")
fig1 = plt.figure()
...
plt.title(f"{ticker} (Close) — Buys (^) and Sells (v)")
plt.xlabel("Date"); plt.ylabel("Price (USD)")
st.pyplot(fig1, use_container_width=True)

st.subheader("Average Cost Basis Over Time")
fig_cb = plt.figure()
plt.plot(details["ind"].index, details["ind"]["Close"], label=f"{ticker} Close")
...
def render_block(sim_summary, sim_details, label_prefix: str, key_prefix: str):
    ...
    spot_now, spot_src2 = (fetch_spot_eth_usd() if ticker == "ETH-USD" else fetch_spot_yahoo_close(ticker))
    d2.metric(f"{ticker} price (now)", f"{format_usd(spot_now) if pd.notna(spot_now) else '—'}" + (f" · {spot_src2}" if spot_src2 and spot_src2 != "—" else ""))

    st.subheader(f"{label_prefix} {ticker} Price with Buys & Sells")
    figp = plt.figure()
    ...
    plt.title(f"{label_prefix} {ticker} (Close) — Buys (^) and Sells (v)")
    plt.xlabel("Date"); plt.ylabel("Price (USD)")
...
if ind_bench.empty:
    ...
    with tabs[0]:
        ...
        render_block(sum_bh, det_bh, label_prefix="Buy & Hold —", key_prefix=f"bh_{ticker}")
    ...
    with tabs[1]:
        ...
        render_block(sum_dca, det_dca, label_prefix="Monthly DCA —", key_prefix=f"dca_{ticker}")
...
def run_strategy_for_ticker(tkr: str):
    ...
    # non-ETH reference price default: start-date close
...
def load_close_series_for_all(end_dt: date, years: int):
    ...
    for tk in ASSETS.keys():
        df = fetch_ohlc(start_dt, end_dt, source=data_source, ticker=tk)
        if df.empty:
...
- Data: ETH uses Coinbase/Kraken/Binance/Yahoo with fallback; GLD/^SOX/ARTY use Yahoo Finance.
